class Impressao {
  static async imprimir(idRelatorio, idOrdemServico, idOrdemServicoExterna) {
    ...
    }/mge/service.sbr?serviceName=${`VisualizadorRelatorios.visualizarRelatorio`}&outputType=json&mgeSession=${JX.getCookie(
      "JSESSIONID"
    ).replace(".master", "")}`;
  ...
  static async buscarRelatorioImpressao(nomeTabela) {
    ...
    else throw new Error("Não foi possível encontrar o relatório de impressão");
  }
}
